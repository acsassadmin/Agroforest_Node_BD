...
exports.sendLoginOtp = async (req, res) => {
  try {
    ...
    const [userRows] = await db.query(
      ...
      LEFT JOIN users_role r 
        ON r.id = u.role_id

      WHERE RIGHT(
        REPLACE(REPLACE(u.phone,'+91',''),' ',''),
        10
      ) = ?
      `,
      [cleanPhone],
    ...
    const allowedProductionRoles = ["production_center"];

    // OFFICER LOGIN
    if (expected_role_group === "officer") {
      if (!allowedOfficerRoles.includes(user.role_name)) {
        return res.status(403).json({
          message: "Access Denied: Officer login only",
        });
      }
    }
      ...
      if (username && username.trim() !== "") {
        const [rows] = await db.query(
          ...
          LEFT JOIN users_customuser u 
            ON u.id = pc.created_by_id

          LEFT JOIN users_role r 
            ON r.id = u.role_id

          WHERE pc.created_by_id = ?

          AND EXISTS (
              SELECT 1 
              FROM production_user_codes puc
              WHERE puc.username = ?
              AND RIGHT(
                REPLACE(REPLACE(puc.mobile_number,'+91',''),' ',''),
                10
              ) = ?
          )
          `,
          [user.id, username, cleanPhone],
        );
    ...
    if (expected_role_group === "production_center") {
      smsPayload = `Dear ${finalData.name_of_production_centre || "User"},
...
- Team`;
      ...
      smsPayload = `Dear ${user.username || "Officer"},
...
- Team`;
    ...
    // await sendOtpSms(formattedPhone, smsPayload);
    ...
    const payload = {
      ...
      // LOGIN GROUP
      role_group: expected_role_group,

      user_name: user.username,

      production_center_id: finalData.production_center_id || null,

      production_center_status: finalData.production_center_status || null,

      production_type: finalData.production_type || null,

      // LOCATION DATA
      department_id: finalData.department_id || user.department_id || null,

      district_id: finalData.district_id || user.district_id || null,

      block_id: finalData.block_id || user.block_id || null,

      village_id: finalData.village_id || user.village_id || null,
    };
    ...
    await redisClient.set(`login_${formattedPhone}`, JSON.stringify(payload), {
      EX: 600,
    });
...
exports.verifyLoginOtp = async (req, res) => {
  try {
    ...
    console.log("data otp:", data?.otp, "otp", otp);
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    // Update User in DB
    await db.query("UPDATE users_customuser SET password = ? WHERE phone = ?", [
      hashedPassword,
      storedPhone,
    ]);
...
exports.createFarmer = async (req, res) => {
  try {
    ...
    if (type === "farmer") {
      ...
      return res
        .status(400)
        .json({ error: "Invalid type. Must be 'farmer' or 'non-farmer'." });
    }
  ...
  try {
    ...
    if (aadhar_no) {
      ...
      if (!farmerRows.length) {
        ...
        return res.status(404).json({
          error:
            "Your credentials are not registered. Please complete the registration to proceed.",
        });
      }
...
exports.getAadhar = async (req, res) => {
  try {
    ...
    const refreshToken = jwt.sign({ id: user_id }, JWT_REFRESH_SECRET, {
      expiresIn: "7d",
    });
  ...
  try {
    ...
    if (farmerRows.length === 0) {
      return res.json({
        status: "not_found",
        message:
          "Aadhaar not found in farmer database. Please complete registration.",
        isVerified: false,
      });
    }
    ...
    if (existingUser.length > 0) {
      return res.status(400).json({
        error:
          "Mobile number already linked to another account. Please contact support.",
      });
    }
    ...
    try {
      ...
      if (farmer.block_id) {
        const [block] = await db.query(`SELECT name FROM block WHERE id = ?`, [
          farmer.block_id,
        ]);
...
exports.approveItem = async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const { action, approved_quantity, type, scheme_id, schemed_rate } =
      req.body;
...
exports.getTnSchemas = async (req, res) => {
  try {
    ...
    const cachedData = await redisClient.get("tnSchemes");
    if (cachedData) {
      res.json(JSON.parse(cachedData));
      return;
    }
    const [rows] = await db.query(
      `SELECT id, name FROM tn_schema ORDER BY id ASC`,
    );
    const response = {
      count: rows.length,
      results: rows,
    };
    await redisClient.set("tnSchemes", JSON.stringify(response), { EX: 60 });
    res.json(response);
  } catch (err) {
...
exports.updateRequestHeader = async (req, res) => {
  try {
    ...
    await db.query(`UPDATE users_farmerrequest SET status = ? WHERE id = ?`, [
      status,
      id,
    ]);
  ...
  try {
    ...
    for (const item of approvedItems) {
      ...
      const itemTotalPrice =
        item.approved_quantity * (item.price_per_sapling || 0);
      ...
      if (orderData.length > 0) {
        ...
        if (userRows.length > 0 && userRows[0].phone) {
          ...
          sendBillLinkSms(farmerPhone, orderid, farmerName)
            ...
            .catch((smsErr) =>
              console.error("❌ Bill SMS failed:", smsErr.message),
            );
        } else {
...
exports.getDashboardCounts = async (req, res) => {
  try {
    ...
    // ✅ FIX 1: Dynamic cache key
    const cacheKey = `dashboard:${user.id}:${role}:${department_id || ""}:${district_id || ""}:${block_id || ""}`;

    // ✅ FIX 2: Parse cached data
    try {
    ...
    const getFarmerCount = async () => {
      let query = `SELECT COUNT(*) as count FROM users_farmeraathardetails f`;
      let qParams = [];
    ...
    if (role === "superadmin") {
      data.department_admin_count =
        await getOfficerRoleCount("department_admin");
    ...
    // ✅ FIX 3: non-blocking + correct key
    redisClient
...
exports.getWeeklyFarmerRequestReport = async (req, res) => {
  try {
    ...
    const { start_date, end_date, role, department_id, district_id, block_id } =
      req.query;
...
exports.getFarmerDetails = async (req, res) => {
  try {
    ...
    if (role === "superadmin") {
      ...
      return res
        .status(403)
        .json({ success: false, error: "Unauthorized role access." });
    }
...
exports.getProductionCenterSaplings = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: "Failed to fetch sapling details",
      details: err.message,
    });
  }
};
...
exports.getMonthlyTotalSales = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, year: yy, count: months.length, data: months });
  } catch (err) {
    console.error("❌ Monthly Total Sales Error:", err);
    res.status(500).json({
      success: false,
      error: "Failed to fetch monthly total sales",
      details: err.message,
    });
  }
};
exports.createProductionCenter = async (req, res) => {
  try {
    ...
    // 2️⃣ Check duplicate production code
    ...
    // 3️⃣ OPTIONAL:
    // create user only if mobile not already exists
    ...
    const [userResult] = await db.query(
      `
        ...
        VALUES (?, ?, ?, ?, NOW())
        `,
      [username, mobile, email, 2],
    );

    userId = userResult.insertId;

    // 4️⃣ Save mobile + save username
    await db.query(
      `
      INSERT INTO production_user_codes
...
`,
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename=Bill_${order.orderid || order_id}.pdf`,
    );
    doc.pipe(res);
    ...
    doc
      ...
      .fontSize(18)
      .font("Helvetica-Bold")
      .text(order.pc_name || "Production Center", c, y, {
        width: cw,
        align: "center",
      });
    y = doc.y + 4;
    ...
    doc.text(
      `Payment: ${(order.payment_type || "N/A").toUpperCase()}`,
      rx,
      doc.y + 2,
    );
    ...
    doc
      ...
      .fontSize(11)
      .font("Helvetica-Bold")
      .text(`${grandTotal.toFixed(2)}`, txStart + lblW, y, {
        width: valW,
        align: "right",
      });
    y += 30;
