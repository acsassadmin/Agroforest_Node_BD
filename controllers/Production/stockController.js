...
exports.getDistrictDashboardSummary = async (req, res) => {
    try {
        ...
        // 2. Query for Stats
        // Changed logic: Try to get district_id directly from PC table if possible, 
        // otherwise ensure master_block mapping is correct.
        const statsQuery = `
            SELECT 
                COUNT(DISTINCT pc.id) AS total_production_centers,
                COALESCE(SUM(ps.total_selled), 0) AS total_sales,
        ...
        // 4. Query for Saplings Data
        // Grouping by species to get the count
        const saplingsQuery = `
            SELECT 
                t.s_name, 
            ...
            WHERE pc.district_id = ? 
            GROUP BY t.id, t.s_name
        `;

        // Run in parallel
        const [statsResult] = await db.query(statsQuery, [district_id]);
        const [targetResult] = await db.query(targetQuery, [district_id]);
        const [saplingsResult] = await db.query(saplingsQuery, [district_id]);

        // Construct Payload
        const responseData = {
            district: districtName,
            saplings: saplingsResult, // This will now output [{ s_name: "Name", count: Number }, ...]
            total_production_centers: statsResult[0].total_production_centers || 0,
            total_sales: statsResult[0].total_sales,
        ...
        console.error("Get District Dashboard Summary Error:", err);
        res.status(500).json({ error: err.message });
    }
};
