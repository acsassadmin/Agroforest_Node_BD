  ...
  try {
    const { 
      ...
      end_date, 
      created_by,
      scheme_type,   // "Scheme" or "Non-Scheme"
      scheme_id      // ID or null
    } = req.body;
    ...
    // If scheme_type is "Scheme", scheme_id is REQUIRED.
    // If scheme_type is "Non-Scheme", scheme_id should be null.
    ...
    // 5. Insert Data
    // Note: We use 'scheme_type || "Non-Scheme"' to ensure a default value if frontend sends empty string
    const [result] = await db.query(
      ...
      [
        ...
        end_date, 
        created_by, 
        scheme_type || "Non-Scheme", // Default to Non-Scheme if undefined
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT 
        ...
        td.start_date, 
        td.end_date, 
        td.scheme_type, /* ADDED THIS LINE */
    ...
    exports.updateTargetDepartment = async (req, res) => {
        try {
            ...
            await db.query(
                ...
                WHERE id = ?`,
                [department_id, target_quantity, start_date, end_date, district_id, block_id, production_center_id, scheme_id, id]
            );
