...
exports.createTargetDepartment = async (req, res) => {
    try {
        ...
        // Check if target exists for the same date range
        ...
        if (existing.length > 0) return res.status(400).json({ message: "Target already exists for this department in this date range" });

        const [result] = await db.query(
            `INSERT INTO target_department (role_id, target_tag, target_quantity, start_date, end_date) 
             VALUES (?, ?, ?, ?, ?)`,
            [department_id, target_tag, target_quantity, start_date, end_date]
        );

        res.status(201).json({ message: "Target Department created successfully", target_id: result.insertId });
...
// ===================== GET ALL TARGET DEPARTMENTS (with production center count) =====================
  ...
  try {
    const [rows] = await db.query(
      `SELECT td.id AS target_id,
              ...
              td.start_date,
              td.end_date,
              td.production_center_count  -- include the new column
